...
DEVICE_FUNC uint32_t ShflBflyImpl(uint32_t mask, uint32_t var,
...
DEVICE_FUNC uint32_t ShflIdxImpl(uint32_t mask, uint32_t var, uint32_t srcLane,
                                 uint32_t width) {
  constexpr int WARP_SIZE = 32;
  const uint32_t SHFL_C = ((WARP_SIZE - width) << 8) | (WARP_SIZE - 1);
...
template <typename T>
DEVICE_FUNC T ShflBfly(uint32_t mask, const T& var, uint32_t laneMask,
                       uint32_t width) {
...
template <typename T>
DEVICE_FUNC T ShflIdx(uint32_t mask, const T& var, uint32_t srcLane,
                      uint32_t width) {
  static_assert((sizeof(T) & (sizeof(T) - 1)) == 0,
                "sizeof(T) must be power of 2");
