...
class Conversation:
    ...
    # Each message is either a string or a tuple of (string, List[image_url], image/video).
    ...
    def get_input_tokens(self, prompt, tokenizer):
        self.input_ids = tokenizer(prompt).input_ids
        self.pattern_ids = tokenizer(
            f"{self.sep}\n"
        ).input_ids  # To calculate truncate lengths
        return self.input_ids

    def get_truncate_length(self):
        index = []
        for i, x in enumerate(self.input_ids):
            if x == self.pattern_ids[0]:
                index.append(i)

        truncate_lengths = [0]
        for i in range(0, len(index), 2):
            # if i % 2 == 0:
            truncate_lengths.append(index[i] + 2 - truncate_lengths[-1])

        truncate_lengths.append(len(self.input_ids) - sum(truncate_lengths))
        return truncate_lengths[1:]
    ...
    def get_images_info(self, preprocessor=None, min_pixels=0, max_pixels=0, fps=None):
        image_urls = []
        images_info = {}
        for i, (role, msg) in enumerate(self.messages[self.offset :]):
            if i % 2 == 0:
                if type(msg) is tuple and len(msg[1]) > 1:
                    msg[1][0]
                    for image in msg[1][1:]:
                        image_urls.append(image)

        # if len(image_urls) > 0:
        #     images, images_url, extra_info = self._get_images(
        #         vision_format, image_urls, preprocessor, min_pixels, max_pixels, fps)
        #     images_info = {
        #         "images": images,
        #         "urls": images_url,
        #         "grid_thw": extra_info,
    ...
    def get_preprocess_req(self, min_pixels=4 * 28 * 28, max_pixels=1280 * 28 * 28):
        preprocess_req = []
        vision_format = None
        for i, (role, msg) in enumerate(self.messages[self.offset :]):
            if i % 2 == 0:
                if type(msg) is tuple and len(msg[1]) > 1:
                    vision_format = msg[1][0]
                    if vision_format == "video":
                        image_urls = []
                        for image in msg[1][1:]:
                            image_urls.append(image)
                        ...
                        if (
                            len(image_urls) == 1
                            and image_urls[0].split(".")[-1].lower()
                            not in image_file_formats
                        ):
                            preprocess_req.append(
                                {
                                    "type": vision_format,
                                    "url": image_urls[0],
                                    "min_pixels": min_pixels,
                        ...
                        for image in msg[1][1:]:
                            preprocess_req.append(
                                {
                                    "type": vision_format,
                                    "url": image[7:] if image.startswith("file://") else image,
                                    "min_pixels": min_pixels,
                                    "max_pixels": max_pixels,
                                    "fps": None,
                                }
    ...
    def save_new_images(self, has_csam_images=False, use_remote_storage=False):
        ...
        if isinstance(last_user_message, tuple):
            _text, images = last_user_message[0], last_user_message[1]
