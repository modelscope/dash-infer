...
# @file    basic_example_baichuan_v2.py
#
import os
import copy
import time
...
def create_test_prompt(default_gen_cfg=None, tokenizer=None):
    input_list = [
        "浙江的省会在哪",
    ...
    for i in range(len(input_list)):
        prompt = []
        prompt.extend(tokenizer.encode(input_list[i]))
        prompt.insert(0, 195)  # user_token_id: 195
        prompt.append(196)  # assitant_token_id: 196
...
if __name__ == "__main__":
    ...
    config_file = "../model_config/config_baichuan2_7b.json"
    # config_file = "../model_config/config_baichuan2_13b.json"
    ...
    testcase_path = os.environ.get("DASHINFER_TESTCASE_PATH")
    if testcase_path is None:
        # download from modelscope
        print("Downloading model from modelscope...")
        original_model["model_path"] = download_model(
        ...
        print("$DASHINFER_TESTCASE_PATH=" + str(testcase_path))
        original_model["model_path"] = os.path.join(
            testcase_path, "./testcase", original_model["model_id"]
        )
    ...
    prompt_list, gen_cfg_list = create_test_prompt(
        engine_helper.default_gen_cfg, tokenizer=engine_helper.tokenizer
    )
