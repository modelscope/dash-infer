 ...
 * @file    unary_elementwise_utest.cu
 */
#include <hiednn.h>
...
#define UTEST_UNARY_ELEMENTWISE(TEST_NAME, \
                                ...
                                EXTPARAM_TYPE, \
                                STD_EXPR) \
TEST(Unary_Elementwise_CUDA, TEST_NAME) { \
    unsigned int seed = 0; \
    ...
    IN_TYPE *x_cu; \
    OUT_TYPE *y_cu; \
    cudaMalloc(&x_cu, VECTOR_LENGTH * sizeof(IN_TYPE)); \
    cudaMalloc(&y_cu, VECTOR_LENGTH * sizeof(OUT_TYPE)); \
    cudaMemcpy(x_cu, x, VECTOR_LENGTH * sizeof(IN_TYPE), \
               cudaMemcpyHostToDevice); \
    cudaMemcpy(y_cu, y, VECTOR_LENGTH * sizeof(OUT_TYPE), \
               cudaMemcpyHostToDevice); \
    \
    ...
    CHECK_HIEDNN(hiednnCudaUnaryElementwiseOp( \
        handle, HIE_UNARY_OP, &alpha, x_desc, x_cu, \
        &extParam1, &extParam2, &beta, y_desc, y_cu)); \
    \
    OUT_TYPE y_out[VECTOR_LENGTH]; \
    cudaMemcpy(y_out, y_cu, VECTOR_LENGTH * sizeof(OUT_TYPE), \
    ...
    for (int i = 0; i < VECTOR_LENGTH; ++i) { \
        ...
        ASSERT_LE(std::abs(static_cast<double>(diff)), \
                  std::abs(static_cast<double>(std_val) * 1e-5) + 1e-5); \
    } \
    \
    cudaFree(x_cu); \
    cudaFree(y_cu); \
