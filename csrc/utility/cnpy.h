...
struct NpyArray {
    ...
    template <typename T> const T* data() const { return reinterpret_cast<T*>(&(*data_holder)[0]); }
...
void parse_npy_header(unsigned char* buffer, size_t& word_size, std::vector<size_t>& shape,
                      bool& fortran_order);
...
template <typename T>
std::vector<char> to_npy_or_save(std::string fname, T* data, const std::vector<size_t> shape) {
    std::vector<char> ret = create_npy_header<T>(shape);
    auto nels = std::accumulate(shape.begin(), shape.end(), 1, std::multiplies<size_t>());
    auto nbytes = nels * sizeof(T);
    for (auto i = 0; i < nbytes; i ++) {
        char* p = reinterpret_cast<char*>(data) + i;
        ret.emplace_back(*p);
    }
    if (!fname.empty())
        npy_save(fname, data, shape);
    return ret;
}
...
void npz_save(std::string zipname, std::string fname, const T* data,
    ...
    global_header +=
        (uint32_t)global_header_offset; // relative offset of local file header, since it begins
                                        // where the global header used to begin
    global_header += fname;
