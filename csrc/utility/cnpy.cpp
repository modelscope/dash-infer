...
void cnpy::parse_npy_header(unsigned char* buffer, size_t& word_size, std::vector<size_t>& shape,
                            bool& fortran_order) {
    ...
    std::vector<std::string> shape_vec = util::StringUtil::Split(str_shape, ", ");
    for (auto& s : shape_vec) {
        shape.emplace_back(atoi(s.c_str()));
    }
