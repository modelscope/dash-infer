 ...
 */
template <int S>
__device__ __forceinline__ uint32_t ByteExtract(uint32_t word, int idx);

template <int S>
__device__ __forceinline__ uint32_t ByteExtract<1>(uint32_t word, int idx) {
  uint32_t ret;
...
template <>
__device__ __forceinline__ uint32_t ByteExtract<2>(uint32_t word, int idx) {
  uint16_t lo, hi;
...
template <>
__device__ __forceinline__ uint32_t ByteExtract<4>(uint32_t word, int idx) {
  return word;
}
...
struct alignas(PACK_SIZE * sizeof(T)) WordPackT {
  ...
  __device__ __forceinline__ void Unpack(ComputeT (&ret)[PACK_SIZE]) const {
...
#pragma unroll
  ...
  template <int UNDERLYING_SIZE, typename UnderlyingT = T, typename ComputeT,
            typename ExtractorT>
  __device__ __forceinline__ void Unpack(
      ComputeT (&ret)[PACK_SIZE * UNDERLYING_SIZE],
