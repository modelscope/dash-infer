...
template <typename T>
struct QCacheConfig<span::QuantMode::I8, T> {
  using QuantT = int8_t;
  using ComputeT = float;
  using UnderlyingT = QuantT;
  using ExtractorT = QCacheExtractor<span::QuantMode::I8>;
  static constexpr int UNDERLYING_SIZE = 1;
};

template <typename T>
struct alignas(2 *
               sizeof(typename QCacheConfig<span::QuantMode::I8, T>::ComputeT))
    QuantParam<span::QuantMode::I8, T> {
  using QT = typename QCacheConfig<span::QuantMode::I8, T>::QuantT;
  using CPT = typename QCacheConfig<span::QuantMode::I8, T>::ComputeT;
  ...
  class Builder {
    using CPT = typename QCacheConfig<span::QuantMode::I8, T>::ComputeT;
    using Param = QuantParam<span::QuantMode::I8, T>;
...
template <int BLOCK, int UNROLL, int HEADSIZE>
template <template <typename> typename FUNC>
__device__ __forceinline__ typename QCacheConfig<span::QuantMode::I8,
                                                 T>::ComputeT
QuantParam<span::QuantMode::I8, T>::Builder<BLOCK, UNROLL, HEADSIZE>::reduce(
    const typename QCacheConfig<span::QuantMode::I8, T>::ComputeT (
        &val)[UNROLL],
...
template <typename T>
template <int BLOCK, int UNROLL, int HEADSIZE>
__device__ __forceinline__ QuantParam<span::QuantMode::I8, T>
QuantParam<span::QuantMode::I8, T>::Builder<
