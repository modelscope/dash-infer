    ...
    def _build_graph(self, torch_cfg, derive_type):
        ...
        if self.multigpu_mode != 0:
            ...
            for i in range(cfg.dec_layer):
                ...
                # 0727之前的保守切法
                # self.split_map[prefix + "attention.self.weight"] = VSPLIT
                # self.split_map[prefix + "attention.self.bias"] = VSPLIT
                # self.split_map[prefix + "attention.output.dense.weight"] = VSPLIT
        ...
        for i in range(cfg.dec_layer):
            ...
            if self.multigpu_mode == 0:
                ...
                ffn_out = self.make_gemm_op(
                    prefix + "ffn.output.dense", ffn_chunk.outputs[0], {
                        "with_bias": False,
        ...
        if self.is_generate:
            ...
            for op in graph.ops:
                if op.op_type == "EmbeddingT5":
                    ...
                    ''' TODO: waiting for DecOptMQAI8CacheOp
                    if self.enable_i8cache_mha:
                        op.op_type = "DecOptMQAI8Cache"
                    else:
                        op.op_type = "DecOptMQA"
                    '''
