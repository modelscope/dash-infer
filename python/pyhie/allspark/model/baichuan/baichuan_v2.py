...
class Baichuan_v2(Model):
    ...
    def _build_graph(self, torch_cfg, derive_type):
        ...
        for i in range(cfg.dec_layer):
            ...
            attn_out_gemm = self.make_gemm_op(prefix + "attention.output.dense",
                                 mha.outputs[0], {"with_bias": False})()
            attn_add = Binary(
            ...
            ffn_out = self.make_gemm_op(prefix + "ffn.output.dense", ffn_mul.outputs[0],
                           {"with_bias": False})()
            final_add = Binary(
                prefix + "final_add",
    ...
    def _trans_weight(self, torch_weight, lora_name=None):
        ...
        for key, torch_name in weight_name_map.items():
            ...
            split_mode = NOSPLIT if key not in split_map else split_map[
                key]
