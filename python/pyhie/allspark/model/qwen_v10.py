...
class Qwen_v10(Model):
    ...
    def _build_graph(self, torch_cfg, derive_type):
        ...
        decoder_name_map = {
            ...
            # "ffn.intermediate.dense1.bias": weight_std_names[13],
            "ffn.intermediate.dense2.weight": weight_std_names[14],
            # "ffn.intermediate.dense2.bias": weight_std_names[15],
            "ffn.output.dense.weight": weight_std_names[16],
            # "ffn.output.dense.bias": weight_std_names[17],
            # "rotary.inv_freq": weight_std_names[18],
        }
    ...
    def _trans_weight(self, torch_weight, lora_name=None):
        ...
        for key, torch_name in weight_name_map.items():
            ...
            if self.adapted_quant_method == "GPTQ":
                #only for GPTQ
                if key.rfind("lm_head.weight") != -1:
                    tensor = torch.permute(tensor, (1, 0)).contiguous()
            else:
                #default
                if re.match(r'.*\.attention\.self(\.lora_B)?\.bias', key):
                    tensor_reshape = tensor.reshape(
                        self.model.model_conf.num_heads, 3, tensor.shape[0] //
                        3 // self.model.model_conf.num_heads)
                    tensor = torch.permute(tensor_reshape,
                                           (1, 0, 2)).reshape(-1)
                if re.match(r'.*\.attention\.self(\.lora_B)?\.weight', key):
                    tensor_reshape = tensor.reshape(
                        self.model.model_conf.num_heads, 3, tensor.shape[0] //
                        3 // self.model.model_conf.num_heads * tensor.shape[1])
