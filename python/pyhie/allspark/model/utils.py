...
# def QuantizeWeightA16W8(fdata, qtype="int8", axis=None):
#     ftype = fdata.dtype
#     qmax = float(np.iinfo(qtype).max)
#     qmin = float(np.iinfo(qtype).min)

#     fmax = np.amax(fdata, axis=axis, keepdims=True)
#     fmin = np.amin(fdata, axis=axis, keepdims=True)

#     scale = (fmax - fmin) / (qmax - qmin)
#     scale = np.where(scale <= 1e-5, 1, scale)
#     zero = qmin - fmin / scale

#     res_data = fdata / scale + zero
#     qdata = np.round(np.clip(res_data, qmin, qmax)).astype(qtype)
#     scale = scale.astype(ftype)
#     zero = zero.astype(ftype)
...
#     fdata = cp.asarray(fdata)
#     fmax = cp.amax(fdata, axis=axis, keepdims=True)
#     fmin = cp.amin(fdata, axis=axis, keepdims=True)

#     scale = (fmax - fmin) / (qmax - qmin)
#     scale = cp.where(scale <= 1e-5, 1, scale)
#     zero = qmin - fmin / scale

#     res_data = fdata / scale + zero
#     qdata = cp.round(cp.clip(res_data, qmin, qmax)).astype(qtype)
#     scale = scale.astype(ftype)
#     zero = zero.astype(ftype)

#     qdata = cp.asnumpy(qdata)
#     scale = cp.asnumpy(scale)
#     zero = cp.asnumpy(zero)
#     return qdata, scale, zero
